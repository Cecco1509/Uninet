<script lang="ts">
  ...
  async function uploadCurrentPhoto(): Promise<string> {
    ...
    let url =
      "groupsInfosImages/" + uuidv4() + "." + avatar[0].type.split("/")[1];
  ...
  async function createGroup() {
    ...
    partecipantsUsers.forEach((user) => {
      partecipanti.push(user!.Username);
    });
    const result = await ChatCache.getCache().addGroupChat(
      newGroupName,
      partecipanti,
  ...
  let popup = $state<
    { result: { esito: boolean; message: string }; i: number } | undefined
  >();
</script>
...
<div class="outer-mdl" bind:this={modal}>
  <div class="inner-mdl">
    <div
      style="height: max-content; display:flex; justify-content:space-between; margin-bottom:10px;"
    >
      <span class="title">Crea un gruppo</span>
      <button
        class="crea"
        onclick={createGroup}
        disabled={partecipantsUsers.length < 2 || !newGroupName}
      >
        {#if creating}
          <LoadIcon />
        {:else}
    ...
    <div class="create-inputs">
      <div style="margin: 20px 0px;">
        ...
        <div
          style="display: flex; align-items:center; margin-top:10px; gap: 10px;"
        >
                ...
                <div class="usr-cnt">
                  <div class="info">
                    ...
                    <div>
                      ...
                      <span class="flws">
                        • {element?.Followers}
                        <!-- {element.n_partecipants} --> Followers
                      </span>
                    </div>
                  </div>
                  <div>
                    <button
                      onclick={() => {
                        partecipantsUsers.push(element);
                        clear();
                      }}
                      >Aggiungi
                    </button>
        ...
        {#each partecipantsUsers as user}
          <div class="usr-cnt">
            ...
            <div>
              <button
                onclick={() =>
                  (partecipantsUsers = partecipantsUsers.filter(
            ...
            <div class="usr-cnt">
              <div class="info">
                ...
                <div>
                  ...
                  <span class="flws">
                    • {element?.n_partecipants}
                    <!-- {element.n_partecipants} --> Partecipanti
                  </span>
                </div>
              </div>
              <div>
